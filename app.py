...
def create_word(evaluation_text, img_bytes, event, date, goal_list):
    ...
    for line in lines:
        ...
        p = doc.add_paragraph()
        # 項目見出しのみを太字にする
        is_heading = any(line.startswith(key) for key in ["【分析】", "【今後の課題・改善案】", "【まとめ】"])
    ...
    if st.button("報告書原稿を作成する"):
        with st.spinner("データを分析中..."):
            ...
            if not wakati.strip():
                ...
                ### 記述のルール
                1. 直接「【分析】」から書き始めてください。
                2. 以前の形式（「ねらい1について、...」と文章で記述する形式）で作成してください。
                3. 各ねらいごとの反応を丁寧に分析し、その後「【今後の課題・改善案】」、「【まとめ】」の順で記述してください。
                4. 語尾は「〜である」「〜した」等の公的文書の体裁にし、記号（#や*）は使用しないでください。
                """
